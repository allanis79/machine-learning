...
grspitnorm(:,5) = 1-grspitnorm(:,5); %flip values to show largest as most successful
...
if filter_condition == 1
    ...
    [filtdata1,filtstdev,filtsterr] = matave([filter1(:,1) filter1(:,2) filter1(:,9) filter1(:,10) filter1(:,11)]);
elseif filtercondition == 2
else
end

%----------Sort out false positive/negative-
cgrcrit = .8;                         %Grasp Data Cutoff Point
cmtcrit = .62;                         %Mechanical Turk Data Cutoff Point
%Unfiltered
[cfn,cfp,ctn,ctp,cct] = truefalse([compdata(:,5) compdata(:,3)],cgrcrit,cmtcrit);
%Filtered
[ffn1,fcfp1,fctn1,fctp1,fcct1] = truefalse([filtdata1(:,5) filtdata1(:,3)],cgrcrit,cmtcrit);

%Survey options for x and y cutoffs ROC Curve Generation
[ccritmtur,fitmtur,statmtur] = roc(.8,.001,filtdata1(:,5),filtdata1(:,3));
[ccriteps,fiteps,stateps] = roc(.8,.001,filtdata1(:,3),grspitnorm(:,3));
[ccritvol,fitvol,statvol] = roc(.8,.001,filtdata1(:,3),grspitnorm(:,2));
[ccritenergy,fitenergy,statenergy] = roc(.8,.001,filtdata1(:,3),grspitnorm(:,5));
...
AUCprep = [ccriteps(:,6) ccriteps(:,7) ccriteps(:,1)];
AUCprep = sort(AUCprep,1);
AUC = 0;
for i = 1:length(AUCprep)-1
    AUC = AUC + (AUCprep(i+1,1)-AUCprep(i,1))*mean([AUCprep(i,2) AUCprep(i+1,2)]);
...
AUCprep = [ccritvol(:,6) ccritvol(:,7) ccritvol(:,1)];
AUCprep = sort(AUCprep,1);
AUC = 0;
for i = 1:length(AUCprep)-1
    AUC = AUC + (AUCprep(i+1,1)-AUCprep(i,1))*mean([AUCprep(i,2) AUCprep(i+1,2)]);
...
AUCprep = [ccritenergy(:,6) ccritenergy(:,7) ccritenergy(:,1)];
AUCprep = sort(AUCprep,1);
AUC = 0;
for i = 1:length(AUCprep)-1
    AUC = AUC + (AUCprep(i+1,1)-AUCprep(i,1))*mean([AUCprep(i,2) AUCprep(i+1,2)]);
...
hold on
%plot(ccriteps(:,7),ccriteps(:,6),'-.','linewidth',1.5,'Color',[.5 .5 .5])
plot(ccritvol(:,6),ccritvol(:,7),'--','linewidth',.75,'Color',[.75 .75 .75])
plot(ccritenergy(:,6),ccritenergy(:,7),'-.','linewidth',1.5,'Color',[.5 .5 .5])
plot([0 1],[0 1],'Color',[.25 .25 .25])

% plot([ccritmtur(statmtur,6) ccritmtur(statmtur,6)+2],[fit(statmtur) fit(statmtur)-2],'g')
legend('Mechanical Turk Data','Volume','Grasp Energy','Random Guess',4)
hold off
...
xlswrite('rocener.csv',ccritenergy(:,6:7))
xlswrite('rocmenerfit.csv',fitenergy)
xlswrite('rocenerinter.csv',statenergy)
